...
class Button():
    ...
    def press(self):
        ...
        global entitys, entitys_cords, player_pass, temp_list_sound, temp_list2_sound, sound_var, entitys_org, player_pass2, entitys2
...
click = {'layer':'fg','choice':grass,'place':False,'destroy':False,'collide':False} #used for clicking#
scroll = {'up':False,'down':False,'left':False,'right':False,'shift':False,'x':0,'y':0} #used for camera#
...
def pos_chunk_function_game():
    global pos_chunk_x, pos_chunk_y, player_value
    pos_chunk_x = int((player_value.x) / 672)
    pos_chunk_y = int((player_value.y) / 672)
...
def instructions():
    ...
    while True:
        ...
        screen['display'].blit(instructions_img,(0,32))
...
def instructions2():
    ...
    while True:
        ...
        screen['display'].fill((255,255,255))
        screen['display'].blit(instructions2_img,(0,32))
        screen['display'].blit(button_menu,(0,0))
        screen['display'].blit(exit_button.image,(exit_button.x,exit_button.y))
        pygame.display.update()
        screen['fps'].tick_busy_loop(60)
...
def editor():
    ...
    buttons = [trash,save,load,play,bg,fg,helpp,sound]
    global place,grass,player,grid,clock,scroll,click,grid,entitys,pos_,entitys_cords,objects, sound_var
    ...
    while True:
        ...
        if click['place'] == True:
            ...
            if click['collide'] != True:
                dummy_false_var = False
                if click['choice'].x >= 0 and click['choice'].x < 672 - scroll['x'] and click['choice'].y >= 0 and click['choice'].y < 672 - scroll['y']:
                    for value in entitys[str(pos_chunk_x)+','+str(pos_chunk_y)]:
                        if value.name == click['choice'].name and value.name == 'player':  #if there is already a player on screen, don't append a new one#
                            dummy_false_var = True
                    if dummy_false_var != True:
        ...
        for value in entitys_cords:
            for value2 in value:
                if value2.x + scroll['x'] > 671 or value2.x + scroll['x'] < 0 or value2.y + scroll['y'] > 671 or value2.y + scroll['y'] < 0:
                    ...
                    if value2.typee == 'PASS':
                        screen['display'].blit(value2.image_transparent,(value2.x + scroll['x'], value2.y + scroll['y']+32))
        ...
        if pos_[1] < 32 or pos_[0] > 671: #if mouse is in editor, display image that follows the mouse#
            ...
            screen['display'].blit(click['choice'].image ,(click['choice'].x + scroll['x'], click['choice'].y + scroll['y']+32))
...
def player_input(player_value):
    global event, scroll, org_y, gravity_var
    try:
        if event.type == pygame.KEYDOWN:
            if event.key == K_LSHIFT:
...
def gravity():
    global org_y, scroll, gravity_var, player, player_value, value2, temp_collide, temp_value, gravity_var
    temp_collide = False
    temp_value = None

    if value2.name != 'player':
        if pygame.Rect(player_value.x,player_value.y+1,player_value.width,player_value.height).colliderect(pygame.Rect(value2.x,value2.y-1,value2.width,value2.height)) and value2.typee == 'SOLID':
            player.move['ground'] = True
            gravity_var = False

        if pygame.Rect(player_value.x,player_value.y+8,player_value.width,player_value.height).colliderect(pygame.Rect(value2.x,value2.y,value2.width,value2.height)) and value2.typee == 'SOLID':
            temp_value = value2
            temp_collide = True
            gravity_var = False
            
        if player_value.y > 5056:
            temp_value = None
            temp_collide = True
            gravity_var = False
...
def game():
    global event, scroll, org_y, gravity_var, entitys, player_pass, player_value, value2, temp_collide, temp_value, gravity_var

    buttons = [edit,helpp2,sound]
    #centers the camera around the player#
        
    scroll['x'] = 384 - player_pass.x 
    scroll['y'] = 320 - player_pass.y
    ...
    while True:
        ...
        for value2 in temp_list:
            if player.move['jump'] == True:
                if value2.name != 'player':
                    if pygame.Rect(player_value.x,player_value.y-8,player_value.width,player_value.height).colliderect(
                        pygame.Rect(value2.x,value2.y+value2.height,value2.width,-1)) and value2.typee == 'SOLID':
                        player_value.y -= (player_value.y - value2.y - value2.width)
                        scroll['y'] += (player_value.y - value2.y - value2.width)
                        gravity_var = True
        ...
        for value2 in temp_list:
            if value2.name != 'player':
                #if the player runs into a wall#
                if pygame.Rect(player_value.x+8,player_value.y,player_value.width,player_value.height).colliderect(pygame.Rect(value2.x,value2.y,value2.width,value2.height))and value2.typee == 'SOLID':
                    if player.move['right'] == True:
                        scroll['x'] -= ((value2.x)-(player_value.x+player_value.width))
                        player_value.x += ((value2.x)-(player_value.x+player_value.width))
                ...
                if pygame.Rect(player_value.x-8,player_value.y,player_value.width,player_value.height).colliderect(pygame.Rect(value2.x,value2.y,value2.width,value2.height)) and value2.typee == 'SOLID':
                    if player.move['left'] == True:
                        scroll['x'] += ((((player_value.x+player_value.width-8)-(value2.x+value2.width))))
                        player_value.x -= ((((player_value.x+player_value.width-8)-(value2.x+value2.width))))
        ...
        for value in entitys_cords:
            for value2 in value:
                if value2.x + scroll['x'] < -64 or value2.x + scroll['x'] > 800 or value2.y + scroll['y'] < -64 or value2.y + scroll['y'] > 800:
                    pass
        ...
        if player.move['jump'] != True:
            if temp_collide != True and gravity_var == True:
                player_value.y += 8
                scroll['y'] -= 8

            if temp_collide == True and gravity_var == True:
                player_value.y += (player_value.y + player_value.height - temp_value.y)
                scroll['y'] -= (player_value.y + player_value.height  - temp_value.y)

            
        if player.move['jump'] == True:
            if org_y > player_value.y + 116: #if player reaches a certain height then stop#
                player_value.y = int(player_value.y)
                ...
                gravity_var = False
                player_value.y -= 8 ; player_value.y = round((player_value.y),2) #rounded because python no like math, and gave me wonky numbers#
                scroll['y'] += 8 ; scroll['y'] = round((scroll['y']),2)


                
        fps = font.render(str(int(screen['fps'].get_fps()))+ ' FPS', True, pygame.Color('black'))
        pos_x = font.render('X: ' +str(player_value.x)+ '/' +str(screen['x_end']), True, pygame.Color('black'))
        pos_y = font.render('Y: ' +str(player_value.y)+ '/' +str(screen['y_end']), True, pygame.Color('black'))
