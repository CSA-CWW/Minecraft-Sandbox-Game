...
def pos_chunk_function_game():
    ...
    pos_chunk2_x = int((player_value.x) / 672) ;  pos_chunk2_x2 = int((player_value2.x) / 672)
    pos_chunk2_y = int((player_value.y) / 672) ;  pos_chunk2_y2 = int((player_value2.y) / 672)
        ...
        if click['place'] == True:
            ...
            if click['collide'] != True:
                if click['choice'].x >= 0 and click['choice'].x < 672 - scroll['x'] and click['choice'].y >= 0 and click['choice'].y < 672 - scroll['y']:
                    for value in entitys[str(pos_chunk_x)+','+str(pos_chunk_y)]:
                        if value.name == click['choice'].name and value.name == 'player' and player_value == None:  #if there is already a player on screen, don't append a new one#
                            pass
